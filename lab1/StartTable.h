...
#ifdef StartTable_ENCODE_MULTIBYTE_STRINGS
#include <clocale>
#ifndef StartTable_USE_EN_US_UTF8
#define StartTable_USE_EN_US_UTF8
#endif
#endif
...
public:
    ...
    StartTable(char horizontal, char vertical, char corner)
        : _horizontal(horizontal), _vertical(vertical), _corner(corner),
        _has_ruler(true) {}

    explicit StartTable(char vertical)
...
#ifdef StartTable_ENCODE_MULTIBYTE_STRINGS
#ifdef StartTable_USE_EN_US_UTF8
        std::setlocale(LC_ALL, "en_US.utf8");
#else
#error You need to specify the encoding if the StartTable library uses multybyte string encoding!
#endif
        unsigned int u = 0;
...
inline std::ostream& operator<<(std::ostream& stream, const StartTable& table) {
    if (table.rows().empty()) {
        return stream;
    }
    table.setup();
    ...
    for (auto rowIterator = table.rows().begin();
        ...
        StartTable::Row const& row = *rowIterator;
        stream << table.vertical();
        for (unsigned i = 0; i < row.size(); ++i) {
            auto alignment = table.alignment(i) == StartTable::Alignment::LEFT
                ? std::left
                : std::right;
